...
export default function Navbar({ mode = 'home' }: { mode?: NavbarMode }) {
  ...
  const [activeSection, setActiveSection] = useState(mode === 'home' ? 'hero' : '')
  ...
  useEffect(() => {
    setActiveSection(mode === 'home' ? 'hero' : '')
  }, [mode])
  ...
  useEffect(() => {
    ...
    const pick = () => {
      ...
      setActiveSection(best ? best[0] : mode === 'home' ? 'hero' : '')
    }
  ...
  return (
    <header className="fixed left-0 right-0 top-4 z-[100] px-6">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center">
        ...
        {/* ── Links bubble (centrée visuellement) ────── */}
        <nav
          ref={navRef}
          className="anim-pop-1 relative hidden items-center gap-0.5 justify-self-center rounded-full px-2 py-2 lg:flex"
